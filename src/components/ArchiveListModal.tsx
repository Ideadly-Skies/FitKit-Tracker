...
import { ArchivedPatient, ArchiveView } from '../types';
import { today, fmtDisplayDate } from '../utils';
...
interface ArchiveListModalProps {
  ...
  setArchiveListOpen: (v: boolean) => void;
  archiveView: ArchiveView;
  setArchiveView: Dispatch<SetStateAction<ArchiveView>>;
}

function exportArchiveCSV(rows: ArchivedPatient[]) {
  const hdr = ["Reference ID", "Patient Name", "Phone", "Result", "Result Date", "Archived On", "Archive Note"];
...
function pagerPages(cur: number, total: number): (number | string)[] {
...
export default function ArchiveListModal({ archived, archiveListOpen, setArchiveListOpen, archiveView, setArchiveView }: ArchiveListModalProps) {
  const [viewPatient, setViewPatient] = useState<ArchivedPatient | null>(null);
  ...
  const inputS: React.CSSProperties = { border: `1px solid ${BR}`, borderRadius: 4, padding: "6px 10px", fontSize: 12, fontFamily: FONT, color: TX, background: "#fff" };
  ...
  return (
                  ...
                  <tbody>
                    {pageRows.length === 0 ? (
                      ...
                      <tr key={p.id} style={{ borderBottom: `0.5px solid ${BR}` }}>
                        ...
                        <td style={{ padding: "8px 12px" }}>{fmtDisplayDate(p.resultDate)}</td>
                        <td style={{ padding: "8px 12px", color: TXM }}>{fmtDisplayDate(p.archivedAt)}</td>
                        ...
                        <td style={{ padding: "8px 12px" }}>
                          <button onClick={() => setViewPatient(p)} style={{ background: "none", border: `1px solid ${BR}`, borderRadius: 4, padding: "3px 10px", fontSize: 11, color: TXM, cursor: "pointer", fontFamily: FONT, fontWeight: 600, whiteSpace: "nowrap" }}>View</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 4, marginTop: 12 }}>
                  ...
                  {pagerItems.map((it, i) => (
                    it === "…" ? (
                      ...
                      <button key={it} onClick={() => setArchiveView(v => ({ ...v, page: it as number }))} style={{ background: it === currentPage ? P : "#fff", color: it === currentPage ? "#fff" : TXM, border: `1px solid ${it === currentPage ? P : BR}`, padding: "6px 10px", borderRadius: 4, cursor: "pointer", fontSize: 11, fontFamily: FONT, fontWeight: 700, minWidth: 30 }}>{it}</button>
                    )
                  ))}
    ...
    <ArchiveViewModal patient={viewPatient} onClose={() => setViewPatient(null)} />
    </>
  );
}
