...
import { ArchiveModalState } from '../types';
import { P, BR, LAV2, TX, TXL, FONT } from '../constants';

interface ArchiveModalProps {
  archiveModal: ArchiveModalState | null;
  setArchiveModal: Dispatch<SetStateAction<ArchiveModalState | null>>;
  onSave: () => void;
}

export default function ArchiveModal({ archiveModal, setArchiveModal, onSave }: ArchiveModalProps) {
    ...
    <Modal title="Archive Patient" onClose={() => setArchiveModal(null)} onSave={onSave} saveLabel="Confirm Archive">
      ...
      <Field label="Archive note (optional)">
        <textarea
          ...
          onChange={e => setArchiveModal(m => m ? ({ ...m, note: e.target.value }) : m)}
