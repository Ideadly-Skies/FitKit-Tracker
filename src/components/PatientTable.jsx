...
import { P, BR, LAV2, TX, TXL, TXM } from '../constants';
    ...
    <div style={{ padding: "16px 20px" }}>
      ...
      <div style={{ background: "#fff", border: `0.5px solid ${BR}`, borderRadius: 8, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <tbody>
            {activeFilter === "positive" ? (
              renderPositiveRows(filtered, setActiveFilter, onArchivePatient, onEditPatient, BR, LAV2, TXL, TXM)
            ) : (
              stagesToShow.map(s => renderStageRows(s, filtered, expandedStages, setExpandedStages, daysPending, onEditPatient, onAdvancePatient, onWithdrawPatient, onArchivePatient, onUploadCSV, actBtn, BR, LAV2, TX, TXL, TXM, P))
            )}
          </tbody>
        </table>
      </div>
...
function renderPositiveRows(filtered, setActiveFilter, onArchivePatient, onEditPatient, BR, LAV2, TXL, TXM) {
...
function renderStageRows(s, filtered, expandedStages, setExpandedStages, daysPending, onEditPatient, onAdvancePatient, onWithdrawPatient, onArchivePatient, onUploadCSV, actBtn, BR, LAV2, TX, TXL, TXM, P) {
      ...
      <td colSpan={10} style={{ padding: "8px 14px", borderBottom: `1px solid ${BR}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={actBtn({ borderColor: P, color: P, fontWeight: 700 })} onClick={() => onUploadCSV(s.id)}>↑ Upload CSV</button>
          </div>
        </div>
      </td>
    </tr>,
        ...
        <td style={{ padding: "8px 12px", whiteSpace: "nowrap" }} onClick={e => e.stopPropagation()}>
          {p.stage < 4 ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" }}>
              ...
              <span onClick={() => onWithdrawPatient(p)} style={{ color: "#791F1F", fontSize: 11, cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2 }}>Withdraw</span>
            </div>
          ) : (
            <span onClick={() => onArchivePatient(p)} style={{ color: TXM, fontSize: 11, cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2 }}>Archive</span>
          )}
