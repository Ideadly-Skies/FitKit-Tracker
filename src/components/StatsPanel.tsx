import { Patient, ArchivedPatient } from '../types';
...
export default function StatsPanel({ patients, archived, activeFilter, onFilterPositive, onClearFilter, onOpenArchive }: StatsPanelProps) {
  ...
  const otherResult = patients.filter(p => ["Negative", "Inconclusive", "Early Terminated", "Rejected", "Not Shared"].includes(p.result)).length;

  const rollupTiles: TileConfig[] = [
    { n: archiveCount, l: "Archive", kind: "rollup", onClick: onOpenArchive, clickable: true },
    { n: activeTotal, l: "Active test", kind: "rollup" },
  ];
  const breakdownTiles: TileConfig[] = [
