...
import { UploadModalState, UploadState, Stage } from '../types';
import { P, BR, LAV2, TXL } from '../constants';

interface UploadModalProps {
  uploadModal: UploadModalState | null;
  setUploadModal: (v: UploadModalState | null) => void;
  uploadState: UploadState;
  setUploadState: (v: UploadState) => void;
  onFileUpload: (file: File, stageId: Stage) => void;
  onConfirmUpload: (stageId: Stage) => void;
  onDownloadTemplate: (stageId: Stage) => void;
}

export default function UploadModal({ uploadModal, setUploadModal, uploadState, setUploadState, onFileUpload, onConfirmUpload, onDownloadTemplate }: UploadModalProps) {
  if (!uploadModal) return null;
  const { stageId } = uploadModal;
  const btnS: React.CSSProperties = { background: P, color: "#fff", border: "none", padding: "7px 14px", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700 };
  const recordCount = uploadState.parsedRows?.length ?? 0;
  ...
  return (
    <Modal
      ...
      saveLabel={`Import ${recordCount} record${recordCount !== 1 ? "s" : ""}`}
    >
      ...
      <div
        onClick={() => (document.getElementById("csv-upload-inp") as HTMLInputElement | null)?.click()}
